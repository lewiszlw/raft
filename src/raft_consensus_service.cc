...
raft::AppendEntriesResp RaftConsensusServiceClient::AppendEntries(const raft::AppendEntriesReq request, const raft::Server server) {
    std::cout << "RaftConsensusServiceClient::AppendEntries request " << request.DebugString() << ", " << server.address() << std::endl;
    ...
    std::cout << "RaftConsensusServiceClient::AppendEntries response " << response.DebugString() << ", " << server.address() << std::endl;
    if (status.ok()) {
        return response;
    } else {
        // TODO error handling
...
raft::RequestVoteResp RaftConsensusServiceClient::RequestVote(const RequestVoteReq request, const Server server) {
    std::cout << "RaftConsensusServiceClient::RequestVote request " << request.DebugString() << ", " << server.address() << std::endl;
    std::unique_ptr<raft::RaftConsensusService::Stub> stub = NewStub(server);

    RequestVoteResp response;
    grpc::ClientContext context;
    grpc::Status status = stub->RequestVote(&context, request, &response);
    std::cout << "RaftConsensusServiceClient::RequestVote response " << response.DebugString() << ", " << server.address() << std::endl;
    if (status.ok()) {
        return response;
    } else {
        // TODO error handling
...
std::unique_ptr<raft::RaftConsensusService::Stub> RaftConsensusServiceClient::NewStub(const Server server) {
...
grpc::Status RaftConsensusServiceImpl::AppendEntries(grpc::ServerContext* context, const AppendEntriesReq* request, AppendEntriesResp* response) {
    std::cout << "Handle AppendEntries request " << request->DebugString() << std::endl;
    handle_append_entries_(request, response);
    std::cout << "Handle AppendEntries response " << response->DebugString() << std::endl;
...
grpc::Status RaftConsensusServiceImpl::RequestVote(grpc::ServerContext* context, const RequestVoteReq* request, RequestVoteResp* response) {
    std::cout << "Handle RequestVote request " << request->DebugString() << std::endl;
    handle_request_vote_(request, response);
    std::cout << "Handle RequestVote response " << response->DebugString() << std::endl;
    return grpc::Status::OK;
}
 ...
 * Raft 节点通信 RPC 服务端
 ******************************************************************************/
RaftConsensusServiceServer::RaftConsensusServiceServer(Server* local_server, RaftConsensusServiceImpl* service_impl) {
